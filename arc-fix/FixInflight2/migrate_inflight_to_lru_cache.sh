...
check_inflight_presence() {
    npm ls inflight 2>/dev/null | grep -q "inflight@" && echo "1" || echo "0"
}
...
add_npm_overrides() {
    ...
    // Override node-gyp to use newer glob
    packageJson.overrides['node-gyp'] = {
        'glob': '^10.3.10'
    };
    ...
    fs.writeFileSync(process.argv[1], JSON.stringify(packageJson, null, 2));
